...
pub async fn get_enrolled_patients_map(pool: &SqlitePool) -> Result<HashMap<String, String>> {
    let rows: Vec<(String, String)> = sqlx::query_as(
        "SELECT hn, status FROM tb_patients"
    )
    .fetch_all(pool)
    .await?;
...
pub async fn enroll_patient(pool: &SqlitePool, input: &EnrollmentInput) -> Result<i64> {
    ...
    let start_date =
        NaiveDate::parse_from_str(&input.treatment_start_date, "%Y-%m-%d")
...
pub async fn get_all_treatment_plans(
    pool: &SqlitePool,
    hn: &str,
) -> Result<Vec<TreatmentPlan>> {
    sqlx::query_as::<_, TreatmentPlan>(
        "SELECT id, hn, regimen, phase, phase_start, phase_end_expected,
                drugs, duration_months, is_current, notes, created_at
         FROM   tb_treatment_plans
...
pub async fn get_first_phase_start(pool: &SqlitePool, hn: &str) -> Result<Option<String>> {
    let date: Option<String> = sqlx::query_scalar(
        "SELECT MIN(phase_start) FROM tb_treatment_plans WHERE hn = ?",
    )
    .bind(hn)
    .fetch_one(pool)
    .await?;
...
pub async fn update_treatment_phase(
    pool: &SqlitePool,
    update: &TreatmentPlanUpdate,
) -> Result<()> {
    let now = Local::now().format("%Y-%m-%dT%H:%M:%S").to_string();

    let phase_start =
        NaiveDate::parse_from_str(&update.phase_start, "%Y-%m-%d")
            .unwrap_or_else(|_| Local::now().date_naive());
...
pub async fn discharge_patient(pool: &SqlitePool, input: &OutcomeInput) -> Result<()> {
    ...
    sqlx::query(
        "UPDATE tb_patients SET status = ?1, updated_at = ?2 WHERE hn = ?3",
    )
    .bind(patient_status)
    .bind(&now)
    .bind(&input.hn)
