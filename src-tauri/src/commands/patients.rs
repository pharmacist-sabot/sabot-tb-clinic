...
use crate::models::patient::{EnrollmentInput, ActivePatientRow, PatientDetail};
...
pub async fn get_patient_detail(
    ...
    let outcome = db::sqlite::get_outcome(&sqlite, &hn)
        .await
        .ok()
        .flatten();
    ...
    if let Some(pool) = mysql_pool {
        ...
        if let Some(plan) = current_plan {
            if plan.phase == "continuation" {
                if let Ok(true) =
                    db::mysql::was_ethambutol_dispensed_recently(pool, hn, 30).await
                {
                    alerts.push(PatientAlert {
                        hn: hn.to_string(),
